...
class TimeEmbedding(nn.Module):
    """
    Input time:(1 , 328) from sinusoidal or learned embedding of timestep 
    convert it to (1,1280) via linear layers
...
class ResidualBlock(nn.Module):
    """
    Captures local image feature via convulation. Injects time-conditioning
    """
...
class AttentionBlock(nn.Module):
    """
    Self-attention captures global image structure
    Cross-attentiono injects text guidance to images

    Input: (B, C, H, W)
    Flatten spatial grid: (B, H*W, C)
    Self-attention: (B, H*W, C) → (B, H*W, C) (captures long-range pixel-to-pixel dependencies).
    Cross-attention: queries from image, keys/values from text (B, Seq_len, d_context)
    Output: (B, H*W, C) aligned with text tokens.
    GeGLU FFN: expand → gate → project back.
    Reshape: (B, C, H, W)
    """
...
class FinalLayer(nn.Module):
    """
    Converts back into latent space dimension 
    (B , 320 , H / 8 , W / 8) -> GroupNorm -> SilU -> Conv -> (B , 4 , H / 8 , W / 8)
    Latent space has 4 channels (VAE encoder compresses RGB image to 4 channels and H = H / 8 , W = W / 8)
    """
